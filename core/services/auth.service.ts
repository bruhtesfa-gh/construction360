...
export class AuthService {
  ...
  async authenticate(user_login_id: string, password: string) {
    ...
    const userPermissions = (user.user_permissions as Array<{ auth_permissions: { codename: string, resource_type: string } }>).map((up) => `${up.auth_permissions.resource_type}:${up.auth_permissions.codename}`);
    const groupPermissions = (user.user_groups as Array<{ auth_groups: { auth_group_permissions: Array<{ auth_permissions: { codename: string, resource_type: string } }> } }>)
      .flatMap((ug) => ug.auth_groups.auth_group_permissions.map((gp) => `${gp.auth_permissions.resource_type}:${gp.auth_permissions.codename}`));
    const permissions = Array.from(new Set([...userPermissions, ...groupPermissions]));
    ...
    const refreshToken = sign({ sub: user.user_id }, JWT_SECRET, { expiresIn: '30d' });
  ...
  async signup(data: SignupInput) {
    ...
    const user = await prisma.users.create({
      data: {
        ...
        role_id: process.env.DEFAULT_ROLE_ID || "00000000-0000-0000-0000-000000000001", // Set a default role if needed
      },
    });
    return { user, builder };
  }
